    ...
    <>
      {filteredItems.map((item) => (
        <div key={item.id} className="my-5 ">
          ...
          >
            <div className="flex CardStylesmx-2 px-1 rounded-[10px] bg-red00   flex-col items-center">

                <div className=" ">
                  <BlurImage item={item.img} 
                  classNames={`duration-700 h-full px-1 rounded-[18px] w-full object-cover hover:slale-110 ease-in-out group-hover:opacity-75`}
                  divNames={`relative w-[366px] mt-2 py-1 rounded-[13px] h-[240px] sm:h-[220px]`}
                />

                <p className="font-Poppins text-xl text-slate-300 bg-slate-600 absolute right-5 bottom-5 rounded-md px-3 py-1">
                  {item.DeliveryTime}
                </p>
              </div>

              <div className="flex  items-center gap-5  justify-between ">
                <p className="text-3xl font-thin font-Poppins p-1">
                  {item.description}
                </p>
                <p>
              ...
              <ul className=" flex-wrap  items-center list-none gap-4 mt-2 flex">
                {item.cuisines.map((cuisine, index) => (
                  <li
                    className="text-[10px]  px-2 py-1 rounded-md text-slate-100 bg-slate-500"
                    key={item.id}
                  >
                    {cuisine}
                  </li>
              ...
              <p className="text-2xl my-2">
                {" "}
                ✅{item.price}+ orders placed recently
              </p>
            </div>
          </Link>
        </div>
